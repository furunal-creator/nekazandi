...
export interface IStorage {
  ...
  // Guest usage
  getGuestUsage(sessionId: string): number;
  incrementGuestUsage(sessionId: string): number;
}
...
export class SqliteStorage implements IStorage {
  ...
  getGuestUsage(sessionId: string): number {
    ...
    return row?.calculationCount ?? 0;
  }

  incrementGuestUsage(sessionId: string): number {
    const existing = db.select().from(guestUsage).where(eq(guestUsage.sessionId, sessionId)).get();
    if (existing) {
      const newCount = (existing.calculationCount ?? 0) + 1;
      sqlite.prepare("UPDATE guest_usage SET calculation_count = ? WHERE session_id = ?").run(newCount, sessionId);
      return newCount;
    } else {
      db.insert(guestUsage).values({ sessionId, calculationCount: 1 }).run();
      return 1;
    }
  }
}
